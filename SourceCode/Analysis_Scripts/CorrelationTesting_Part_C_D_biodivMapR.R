...
############################################################
# Script Name:    CorrelationTesting_Part_C_D_biodivMapR.R
...
clustering_index_name <- 'Unique_Spectral_Species_WCSS'
...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_part_c_d_biodivmapr.png",
    "correlation_habitat_types_part_c_d_biodivmapr.png",
    "correlation_plant_species_part_c_d_biodivmapr.png"
  )
)
...
for (i in 1:nrow(cor_metrics)) {
  ...
  base_plot <- ggplot(metrics_data_df, aes(x = Unique_Spectral_Species_WCSS, y = !!yvar_sym, color = Testsite)) +
    ...
    labs(
      title = paste(metric_label, "vs Spectral Species count group C/D biodivMapR"),
...
write.csv(correlation_results, file.path(output_folder_path, "correlation_summary_part_c_d_biodivmapr.csv"), row.names = FALSE)
...
model_plant_communities_lm <- lm(Unique_Plant_Cummunities ~ Unique_Spectral_Species_WCSS,
               data = lm_model_input)
# anova(model_plant_communities_lm)
model_plant_communities_lm_residuals_path <- file.path(output_folder_path,"model_plant_communities_lm_residuals_part_c_d_biodivmapr.png")
...
model_plant_communities_lm_model_summary_path <- file.path(output_folder_path,"model_plant_communities_lm_model_summary_part_c_d_biodivmapr.txt")
...
model_habitat_types_lm_residuals_path <- file.path(output_folder_path,"model_habitat_types_lm_residuals_part_c_d_biodivmapr.png")
...
model_habitat_types_lm_model_summary_path <- file.path(output_folder_path,"model_habitat_types_lm_model_summary_part_c_d_biodivmapr.txt")
...
model_plant_species_lm_residuals_path <- file.path(output_folder_path,"model_plant_species_lm_residuals_part_c_d_biodivmapr.png")
...
model_plant_species_lm_model_summary_path <- file.path(output_folder_path,"model_plant_species_lm_model_summary_part_c_d_biodivmapr.txt")
