...
interpret_r <- function(r) {
  ...
  # Script: CorrelationTesting_Part_C_D_Custom.R
  # Description: Tests correlation between custom spectral species indices and ecological metrics for parts C and D using various statistical methods and visualizations.
...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_part_c_d_custom.png",
    "correlation_habitat_types_part_c_d_custom.png",
    "correlation_plant_species_part_c_d_custom.png"
  )
)
...
for (i in 1:nrow(cor_metrics)) {
  ...
  base_plot <- ggplot(metrics_data_df, aes(x = Unique_Spectral_Species_WCSS_Custom, y = !!yvar_sym, color = Testsite)) +
    ...
    labs(
      title = paste(metric_label, "vs Spectral Species count group C/D Custom"),
  ...
  file_path <- sub("Custom_", "WCSS_", file_path)
...
write.csv(correlation_results, file.path(output_folder_path, "correlation_summary_part_c_d_custom.csv"), row.names = FALSE)
...
lm_model_input <- metrics_data_df %>%
  select(Testsite,Unique_Plant_Cummunities,Unique_Spectral_Species_WCSS_Custom,Unique_Habitat_Types,Unique_Plant_Species) %>%
...
model_plant_communities_lm_residuals_path <- file.path(output_folder_path,"model_plant_communities_lm_residuals_part_c_d_custom.png")
...
model_plant_communities_lm_model_summary_path <- file.path(output_folder_path,"model_plant_communities_lm_model_summary_part_c_d_custom.txt")
...
model_habitat_types_lm_residuals_path <- file.path(output_folder_path,"model_habitat_types_lm_residuals_part_c_d_custom.png")
...
model_habitat_types_lm_model_summary_path <- file.path(output_folder_path,"model_habitat_types_lm_model_summary_part_c_d_custom.txt")
...
model_plant_species_lm_residuals_path <- file.path(output_folder_path,"model_plant_species_lm_residuals_part_c_d_custom.png")
...
model_plant_species_lm_model_summary_path <- file.path(output_folder_path,"model_plant_species_lm_model_summary_part_c_d_custom.txt")
