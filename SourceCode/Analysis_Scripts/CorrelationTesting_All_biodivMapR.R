...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_biodivmapr.png",
    "correlation_habitat_types_biodivmapr.png",
    "correlation_plant_species_biodivmapr.png"
  )
)
...
for (i in 1:nrow(cor_metrics)) {
  ...
  base_plot <- ggplot(metrics_data_df, aes(x = Unique_Spectral_Species_WCSS, y = !!yvar_sym, color = Testsite)) +
    ...
    labs(
      title = paste(metric_label, "vs Spectral Species Count biodivMapR"),
...
model_plant_communities_lm_residuals_path <- file.path(output_folder_path,"model_plant_communities_lm_residuals_all_biodivmapr.png")
...
model_habitat_types_lm_residuals_path <- file.path(output_folder_path,"model_habitat_types_lm_residuals_all_biodivmapr.png")
...
model_plant_species_lm_residuals_path <- file.path(output_folder_path,"model_plant_species_lm_residuals_all_biodivmapr.png")
