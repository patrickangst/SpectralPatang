...
############################################################
# Script Name:    CorrelationTesting_Part_E_biodivMapR.R
...
cor_metrics <- data.frame(
  ...
  filename = c(
    "correlation_plant_communities_part_e_biodivmapr.png",
    "correlation_habitat_types_part_e_biodivmapr.png",
    "correlation_plant_species_part_e_biodivmapr.png"
  )
)
...
for (i in 1:nrow(cor_metrics)) {
  ...
  base_plot <- ggplot(metrics_data_df, aes(x = Unique_Spectral_Species_WCSS, y = !!yvar_sym, color = Testsite)) +
    ...
    labs(
      title = paste(metric_label, "vs Spectral Species count group E biodivMapR"),
...
write.csv(correlation_results, file.path(output_folder_path, "correlation_summary_part_e_biodivmapr.csv"), row.names = FALSE)
...
model_plant_communities_lm_model_summary_path <- file.path(output_folder_path,"model_plant_communities_lm_model_summary_part_e_biodivmapr.txt")
...
model_habitat_types_lm_model_summary_path <- file.path(output_folder_path,"model_habitat_types_lm_model_summary_part_e_biodivmapr.txt")
...
model_plant_species_lm_model_summary_path <- file.path(output_folder_path,"model_plant_species_lm_model_summary_part_e_biodivmapr.txt")
